/*
 * Copyright (c) 2012. Global Liquid Markets, LLC.
 * All Rights Reserved.
 */

package com.glmx.pass.data;

import com.glmx.pass.dbo.PassDo;
import com.glmx.pass.dbo.enums.PassTypeEnum;
import java.io.IOException;
import org.codehaus.jackson.annotate.JsonIgnore;

/**
 *
 * @author Kelvin Zheng <kelvin@glmx.com>
 */
public class Generic extends Pass {

  public Generic() {
    super();
  }

  public Generic(PassDo pass) throws IOException {
    super(pass);
  }
  
  /**
   * @return the coupon
   */
  public DictionaryKeys getGeneric() {
    return this.fields;
  }

  /**
   * @param coupon the coupon to set
   */
  public void setGeneric(DictionaryKeys coupon) {
    this.fields = coupon;
  }

  @JsonIgnore
  @Override
  public String getPassType() {
    return PassTypeEnum.generic.name();
  }
}
